...
from vf_descartes.msg import VFDescartesAction, VFDescartesGoal
from support_utilities.ArmParameters import ArmParameters
from support_utilities.ArmFunctions import ArmFunctions
...
class VFNavigation:
    ...
    def __init__(self, params):
        ...
        self.vf_nav_descartes = SimpleActionClient("vf_nav_descartes_server",
                                                   VFDescartesAction)
    ...
    def path_nav(self, command):
        ...
        if command == 'stop':
            ...
            # Me.info_message("Command " + command + " received.")
            found = True
            Me.info_message("Poses in path: " + str(len(self.data['poses']['path'].poses)))
            self.vf_nav_descartes.wait_for_server()
            goal = VFDescartesGoal(pose_array=self.data['poses']['path'])
            self.vf_nav_descartes.send_goal(goal)
            self.vf_nav_descartes.wait_for_result()
            # success = \
    ...
    def add_pose(self):
        ...
        # Removed to maintain vf frame in pose array
        '''try:
            # Convert pose to base frame
            self.data['poses']['marker'].header.stamp = \
                self.tfl.getLatestCommonTime(self.params['world'],
        ...
        self.data['poses']['path'].header.frame_id = self.data['poses']['marker'].header.frame_id
        self.data['poses']['path'].header.stamp = Time.now()
        self.data['poses']['path'].poses.append(deepcopy(self.data['poses']['marker'].pose))
